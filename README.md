...
This mandatory version uses one static buffer for one reading state. It is designed for the mandatory part of the project. Managing multiple file descriptors at the same time is part of the bonus requirements and would require a different storage strategy.

The behavior is undefined if the file is modified between calls before `read()` reaches the end of the file. Reading binary files is also outside the expected behavior of the mandatory project.
...
AI was used to help organize this README, and trying to find solutions for edge cases.
